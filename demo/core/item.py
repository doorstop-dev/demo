...
from demo.core import base
from demo.core.base import auto_load, auto_save, BaseFileObject
...
class Item(BaseFileObject):  # pylint: disable=R0904
    ...
    def __init__(self, path, root=os.getcwd()):
        ...
        if not os.path.isfile(path):
            raise DemoError("item does not exist: {}".format(path))
    ...
    def new(path, root, prefix, sep, digits, number, level=None, auto=None):  # pylint: disable=R0913
        ...
        @raise DemoError: if the item already exists
    ...
    def issues(self, document=None, tree=None):
        ...
        if not self.normative and self.links:
            yield DemoWarning("non-normative, but has links")
    ...
    def find_ref(self, root=None, ignored=None):
        ...
        @raise DemoError: when no ref is found
...
def split_id(text):
    ...
    if not match:
        raise DemoError("invalid ID: {}".format(text))
